...
-- 1. Identificar la restricción de clave foránea existente (versión corregida)
DO $$
DECLARE
  constraint_name text;
BEGIN
  -- Corrigiendo la sintaxis para evitar problemas de tipo con @>
  ...
  ELSE
    RAISE NOTICE 'No se encontró restricción entre public.users y auth.users';
  END IF;
END $$;

-- 2. Eliminar la restricción de clave foránea (si existe)
-- Primero intentamos con el nombre estándar
ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_id_fkey;

-- También intentamos eliminar usando el nombre más específico si existe
DO $$
DECLARE
  constraint_name text;
BEGIN
