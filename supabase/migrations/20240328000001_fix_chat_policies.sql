...
-- Políticas para chat_webhooks
CREATE POLICY "webhooks_select"
ON chat_webhooks FOR SELECT
TO authenticated
USING (
    EXISTS (
        SELECT 1 FROM chat_rooms
...
ON chat_webhooks FOR DELETE
TO authenticated
USING (
    created_by = auth.uid()
);

-- Políticas para chat_webhook_logs
CREATE POLICY "webhook_logs_select"
ON chat_webhook_logs FOR SELECT
TO authenticated
USING (
    EXISTS (
        SELECT 1 FROM chat_webhooks w
        JOIN chat_rooms r ON w.room_id = r.id
